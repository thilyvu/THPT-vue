...
} = require("../helper/validation_questionType");
...
const importQuestions = async (req, res) => {
  try {
    console.log(req);
    // const result = await newQuestionType.save();
    return res.status(201).json({
      message: "New question type create successful ",
      success: true,
    });
...
const updateQuestionType = async (req, res) => {
  try {
    ...
    if (result.questionTypeName !== oldQuestionType.questionTypeName) {
      let questionTypenameNotTaken = await questionTypeValidation(
    ...
    const updatedQuestionType = await Object.assign(
      oldQuestionType,
      updateQuestionType
    );
...
const questionTypeValidation = async (questionTypeName) => {
  let oldQuestionType = await QuestionType.findOne({
...
const getQuestionTypeById = async (req, res) => {
  try {
    ...
    if (!likeQuestionType)
      return res.status(400).json({ msg: "QuestionType does not exist." });
